...
In order to evaluate the performance of the [o2r reproducibility service](https://o2r.info/results/), we conduct a load test for the following expected usage scenario.
A journal has published a special issue with Executable Research Compendia ([ERC](https://o2r.info/results/)).
Ten ERC passed the peer review and the journal website has two concurrent readers per ERC.
Three authors still submit their workflows to the special issue ("late submissions").
The submission of a workflow to the o2r reproducibility service is called a **creation session** and the interaction of a reader with an ERC is called an **examination session** and they both put a load on the reproducibility service.

The data collected during these load tests allow operators of the reproducibility service to estimate the required infrastructure and helps developers to improve the performance.
Each section of the document contains the functions required to test the o2r service both in a `local` instance and in the public `remote` demo server at  [o2r.uni-muenster.de](https://o2r.uni-muenster.de/).
...
Following a [distinction from GeeksforGeeks](https://www.geeksforgeeks.org/difference-between-load-testing-and-stress-testing/) we distinguish load testing from stress testing as follows  (cf. also [stress testing on Wikipedia](https://en.wikipedia.org/wiki/Stress_testing)):
...
> **Stress Testing**
> 
> _Stress testing is a type of software testing that verifies the stability and reliability of the system. This test particularly determines the system on its robustness and error handling under extremely heavy load conditions._

The tests in this document do not try to push the system to its limits, but to learn more about the system scalability given a set of pre-defined scenarios with a number of parallel users.
The [o2r project proposal](https://doi.org/10.17879/42149626934) contains the following relevant paragraphs in _Work Package 3.1: Technology_.
...
From the author perspective, the test simulates eight general steps.
Some of these steps correspond to **user interaction (U)** and other steps to process corresponding to **o2r reproducibility service tasks (o2r)**.

1. **Session start (U):** Time from the start of the test to accessing the o2r reproducibility service (e.g., grabbing a coffee, adjusting the chair).

2. **Login pause (U):** Time required to loggin into the o2r service (e.g., remembering the password).

3. **Pause before upload (U):** Time required for the user before submitting a paper into the publication (e.g., checking the files and format).

4. **ERC/workspace upload (o2r):** Time required by the o2r reproducibility service to receive the ERC or workspace submitted by the author. After this steps the compendium is merely a candidate and is not publicly available. This step is part of the [candidate process](https://o2r.info/api/#section/Candidate-process).
...
7. **Execution Pause (U):** As we assume that authors are going to be interested to take a look at the contents of their compendium, this time represents an initial examination of the published ERC before running a Job.

8. **Job Execution (o2r):** Time required by the o2r reproducibility service to run the analysis, i.e., the [job](https://o2r.info/api/#section/Execute-a-compendium), packaged in the compendium. 

User steps durations are adjustable before the test in a configuration file.
The steps corresponding to o2r reproducibility service tasks require a series of interactions with the o2r API service and thus their durations are *not* configurable.
These steps are explained in detail in the [o2r task (o2r)](#o2rtasks) section.

### Examination session {#examination_sessions}

#### Overview

An examination sessions consist of a reader interaction with an already available compendium.
From the reader perspective the test simulates four general steps.
Some steps match the ones from the creation session:
...
2. **Login pause (U):** Time required to login into the o2r service (e.g.,remembering the password).
...
User steps durations are adjustable before the test in a configuration file.
The step corresponding to o2r Tasks require a series of interactions with the o2r API service.
This step is explained in the [o2r Tasks](#o2rtasks) section.
...
It is of interest if any of these sources perform better or worse than others.
For the Zenodo option, only the record ID option is used.
...
The test execution requires two functions, the first one `create_dockerfile` and `run_test`. The first one creates a Dockerfile with the configuration to build the environment of the test and the second one executes the test. The `create_dockerfile` function runs inside the `run_test` as it takes the parameters of the test to generate a specific Docker environment.
...
This includes authentication parameters, configuration files and folders with required files (workspaces).
This Dockerfile copies into the environment the `test_o2r.r` and the `o2r_test/o2r_sessions.R` scripts. 
Both files are required to run the containers.
...
```{r load_test_configuration, class.source='fold-hide' }
...
```{r special_issue_creation, class.source='fold-hide'}
...
```{r special_issue_creation_report, class.source='fold-hide' }
...
```{r users_interaction, class.source='fold-hide' }
...
```{r users_interaction_report, class.source='fold-hide' }
...
```{r save_configuration_files, class.source='fold-hide'}
...
```{r run_test}
# Parameters: Exactly same parameters as create_dockerfile function.
run_test <-
  function(cookie,
           endpoint,
           special_issue_folder,
...
This document is published under a [Creative Commons Attribution 4.0 International](https://creativecommons.org/licenses/by/4.0/) (CC BY 4.0) license at <https://o2r.info/api/evaluation/load_test.html>.
