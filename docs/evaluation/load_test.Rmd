...
1. Upload of either a workspace or an ERC via either direct file upload or from one of two supported public shares
...
It is of interest if any of these sources perform better or worse than others. For the Zenodo option, only the DOI-based identification is used.
...
From the reader perspective the test simulates two main steps (See Author Overview numeration):
...
The test includes a series of functions to conduct the usage scenarios including the required processes divided in 4 groups (0) User Authentication, (1) Upload, (2) Metadata editing (i.e. [Candidate process](http://o2r.info/api/compendium/candidate/)) and (3) Compendium execution.
